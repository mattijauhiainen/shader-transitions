...
"next"), and a pass 2 shader that takes both simultaneously plus a blend factor `uT`.
...
The shader now takes two sets of cell data (`uTextureA`/`uLumaRangeA` and
`uTextureB`/`uLumaRangeB`) plus a blend factor `uT`. Both images share the same cell grid,
